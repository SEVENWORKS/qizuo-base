...
function Paging(paramsObj, callback) {
    ...
    this.degeCount = paramsObj.degeCount || 2;             //当前页前后两边可显示的页码个数（不设置时，默认为3）
    ...
    $('#page_ul').on('click','a',function (e) {
        ...
        callback && callback(that.pageIndex, that.pageSize,that);
    });
    ...
    //初始化的时候使用
    callback && callback(this.pageIndex, this.pageSize,that);    //立即执行回调函数
}
