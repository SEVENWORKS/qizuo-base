...
<script id="baseTpl" type="text/html">
	...
	<tr id="tr{{$index+1}}" onclick="buttonPanel('修改','updateDate(\'${jumpPath}system/sys/dictDo?baseId={{$value.baseId}}\')','删除',
			'deleteData({baseId:{{$value.baseId}}},\'${adminPath}system/dict/delete\',\'#tr{{$index+1}}\')');">
		<td>{{$index+1}}</td>
		<td>{{$value.value}}</td>
		<td>{{$value.label}}</td>
	</tr>
...
<script>
    $(function(){
        ...
        buttonOne('新增',function(){
            pjaxFunc('${jumpPath}system/sys/dictDo',global$frameContainer);
        });
        /** ************************************************************ */
        ...
        function qPage(func,pageNo,pageSize){
            $.post('${adminPath}system/dict/page',{pageNo:pageNo,pageSize:pageSize},function(data){
